...
const Customers = () => {
  ...
  const customerHistory = selectedCustomer
    ? mockServices.filter((s) => s.customerId === selectedCustomer.id)
    : [];
  ...
  return (
    <div className="p-6">
      ...
      <Modal
        ...
        title={`Service History - ${selectedCustomer?.name}`}
        size="lg"
      >
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {customerHistory.map((service) => (
                    <tr key={service.id}>
                      ...
                      <td className="px-4 py-2 text-sm text-gray-900">₹{service.estimatedCost}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
